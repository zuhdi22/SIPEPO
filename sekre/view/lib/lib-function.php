...
Class Fungsi extends KwtConfig{	
		...
		function dbSetup(){
				...
				if(mysql_query("CREATE TABLE ".$this->dbName.".memodb(`kwnum` VARCHAR( 20 ) NOT NULL ,`nominal` INT NOT NULL ,`nama_santri` VARCHAR( 25 ) NOT NULL ,`admin` VARCHAR( 25 ) NOT NULL ,`tglkw` VARCHAR( 20 ) NOT NULL ,`ktrg` TEXT NOT NULL ,PRIMARY KEY ( `kwnum` )) ",$this->conn)){
		...
		function insertData($nominal, $nama_santri, $admin,$ktrg){
			//insert nomor memo terbaru
			$this->ConnectDB();
			$kwnums = $this->KwNums();
			$tanggal = $this->Tanggal('tgl').' '.$this->Tanggal('blnL').' '.$this->Tanggal('THN');
			$sql = "INSERT INTO memodb (kwnum, nominal, nama_santri, admin, tglkw, ktrg) VALUES ('$kwnums','$nominal','$nama_santri','$admin','$tanggal','$ktrg')";
		...
		function fetchData($query,$mode){
			$this->ConnectDB();
			$sql = "SELECT * FROM memodb WHERE kwnum LIKE '%$query%' OR nominal LIKE '%$query%' OR nama_santri LIKE '%$query%' OR tglkw LIKE '%$query%' OR ktrg LIKE '%$query%'";
			$retval = mysql_query($sql, $this->conn);
			...
			while ($row = mysql_fetch_array($retval, MYSQL_ASSOC)){
				...
				"</td><td>".$row['nama_santri']."</td><td>".$row['tglkw']."</td><td>".$row['ktrg']."</td></tr>";
			}
			echo "</table>";
			}
			if($mode == 'line'){
				while ($row = mysql_fetch_array($retval, MYSQL_ASSOC)){
					...
					echo "Pembayar         : ".$row['nama_santri']."<br>";
					echo "Untuk Pembayaran : ".$row['ktrg']."<br>";
					echo "Bulan      : ".$row['bulan']."<br>";
